...
__Please note__: Ajv [version 6](https://github.com/epoberezkin/ajv/tree/beta) with [JSON Schema draft-07](https://json-schema.org/work-in-progress) support is released. Use `npm install ajv@beta` to install.
...
- Ajv implements full JSON Schema [draft 6](https://json-schema.org/) and draft 4 standards:
...
There is no need to explicitly add draft 6 meta schema (https://json-schema.org/draft-06/schema and https://json-schema.org/schema) - it is added by default, unless option `meta` is set to `false`. You only need to use it if you have a changed meta-schema that you want to use to validate your schemas. See `validateSchema`.
...
- _async_: determines how Ajv compiles asynchronous schemas (see [Asynchronous validation](#asynchronous-validation)) to functions. Option values:
  ...
  - `"es7"` - compile to es7 async function. Unless your platform supports them you need to provide `processCode` or `transpile` option. According to [compatibility table](https://kangax.github.io/compat-table/es7/)) async functions are supported by:
    - Firefox 52,
    - Chrome 55,
...
- _validateSchema_: validate added/compiled schemas against meta-schema (true by default). `$schema` property in the schema can either be https://json-schema.org/schema or https://json-schema.org/draft-04/schema or absent (draft-4 meta-schema will be used) or can be a reference to the schema previously added with `addMetaSchema` method. Option values:
